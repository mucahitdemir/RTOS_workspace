...
#ifndef SRC_TASK_HANDLER_C_
#define SRC_TASK_HANDLER_C_
...
void menu_task(void *param)
{
	...
	while(1)
	{
		xQueueSend(q_print,&msg_menu,portMAX_DELAY);
				...
				if(cmd->len == 1)
				{
					option = cmd->payload[0] - 48; //ASCII to number conversion
					switch(option)
					{
						case 0:
...
void rtc_task(void *param)
{
	...
	const char* msg_rtc1 = "========================\n"
		...
		const char *msg_rtc_pm_am = "Enter PM/AM:";
...
void print_task(void *param)
{
	...
	while(1)
	{
		xQueueReceive(q_print, &msg, portMAX_DELAY); /*Receive an item from a queue.*/
